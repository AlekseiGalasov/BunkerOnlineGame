...
const LocalSearch = ({placeholder, imagePath, otherClasses, route, iconPosition = 'left'}: LocalSearchProps) => {
    ...
    return (
        <section
            className={`mt-11 flex min-h-[56px] grow items-center gap-4  rounded-[10px] px-4 ${otherClasses}`}>
            {
                iconPosition === 'left' && imagePath && <Image src={imagePath} alt={'search'} width={24} height={24}/>
            }
            <Input
                className='no-focus paragraph-regular placeholder border-none shadow-none outline-none'
                type='text'
